...
export default function AcceptProjectInvitePage() {
  ...
  const [message, setMessage] = useState("Accepting project invite...");
  const [subMessage, setSubMessage] = useState(
    "Please wait while we confirm your project invitation.",
  );

  useEffect(() => {
    const acceptInvite = async () => {
      try {
        const data = await projectInviteService.acceptProjectInvite(token);
        ...
        const errorMessage =
          err?.response?.data?.message || "Failed to accept project invite";
      ...
      <div className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900/70 p-8 text-center shadow-2xl">
        ...
        {status === "success" && (
          <div className="mt-6 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-xs text-emerald-300">
            Your project access is active now.
          </div>
        )}
